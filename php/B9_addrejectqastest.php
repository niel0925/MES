...
$cardrejcount = Card::rejectCountRef($account,$reference[0]);
...
$currentQty = (int)$quantity - $total;
	...
	$LogBatch->addLogbatch();
		...
		$batchrejcount = Batch::rejectCountserialRef($account,$reference[0]);

	if(intval($Quantity1) == intval($count) + 1){
		$counter = intval($count) + 1;
		echo 'successreject_'.$batchrejcount."_".$counter."_".$serialno[0]."_".$currentQty."_REJECT";

	}else{
		$counter = intval($count) + 1;
		echo 'false_'.$batchrejcount."_".$counter."_".$serialno[0]."_".$currentQty."_REJECT";	
	 }
...
/*	$lotnumber = new Lotnumber();
	...
	$rejectcount = Card::obarejectCountRef($account,$reference[0]);


	if(intval($Quantity1) == intval($count) + 1){
		$counter = intval($count) + 1;
		echo 'successreject_'.$rejectcount."_".$counter."_".$serialno[0]."_".'1'."_".'REJECT';

	}else{
		$counter = intval($count) + 1;
		echo 'false_'.$rejectcount."_".$counter."_".$serialno[0]."_".'1'."_".'REJECT';	
	 }

//echo 'successreject_'.$rejectcount;
