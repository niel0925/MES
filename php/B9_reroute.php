...
if ($motherexist == 'true'){
	...
	if($status == 'REJECT'){
		...
		$lotmother = new Mother($account,$serialno[0]);
		if($lotmother->getLotno() <> ''){
			echo 'error3';	
		}else{
		...
		if($status == 'REJECT'){
			...
			$lotbatch = new Batch($account,$serialno[0]);
			if($lotbatch->getLotno() <> ''){
				echo 'error3';	
			}else{
			...
			if($status == 'REJECT'){
				...
				$lotcard = new Card($account,$serialno[0]);
				if($lotcard->getLotno() <> ''){
					echo 'error3';	
				}else{
					...
					echo 'successReturn_'.$serialno[0].'_'.$lotstation->getStation();
				}
			}
