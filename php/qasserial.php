		...
		if($Serial->getstatus() == 'GOOD'){
			...
			if($Serial->getPartNo() == $model)
			{
				if($NextStation[0] == $station[0]){
					...
					echo 'true_'.$serialno[0]."_".$ModelRoute->getnextstage($account,$Serial->getPartNo(),$ModelRoute->getFlowsequence()).'_'.$Serial->getPartNo().'_'.$type."_".$Serial->getStatus()."_".$Serial->getLastUpdatedBy();
					}else{
