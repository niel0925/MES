...
<input type="hidden" class="form-control" id="mode" name="mode" value="<?php echo $mode; ?>" <?php echo $readonly;?> >
    ...
    <div class="panel panel-primary" style="margin-left:10px;">
                  ...
                  <div class="panel-body">
                      ...
                      <div class="table-responsive" style="overflow-x: scroll;height:250px;">
                            <table class="table table-bordered"  id="tblOrder" name = "tblOrder">
                                <thead>
                                    <tr>
                                        <th class="info">ID</th>
 ...
 <div class="panel panel-primary" id="tblpanel" name = "tblpanel" style="display:none;">
      ...
      <div class="panel-body">  
        <table class="table table-bordered" id="tablechecklist" >
              ...
              <tr class="info">
                    ...
                    <th colspan = "2"><label for="usr" style="text-align: center;">Status   </label></th>   
...
<div class="row" style="margin-top:10px; margin-right: 10px; margin-bottom: 10px;" align="right">
    <div class="col-md-12">
   <button type="button" class="btn btn-info" data-toggle="modal" data-target="Modal_Calibrate">Open Modal</button>
...
//alert(res2[3]);
  ...
  '<td><input  type="text" id="txtsequence[]"  name="txtsequence[]" value="'+res2[0]+'" hidden>'+res2[0]+'</td>'+
    
  '<td><input  type="text" id="txtjoblist[]"  name="txtjoblist[]" value="'+res2[1]+'" hidden>'+res2[1]+'</td>'+
    
  '<td><input  type="checkbox" id="ok[]"  name="ok[]" value="1"> Ok </td>'+
  '<td><input  type="checkbox" id="notok[]"  name="notok[]" value="2"> Not </td>'+

  '<td><input  type="checkbox"  id="replace[]"  name="replace[]" value="" > Replace</td>'+
    
  '<td><input type="text" class="form-control" id ="Remarks[]" name="Remarks[]" placeholder="Remarks" value="" autocomplete="off"></td>'+'</tr>');
...
$( "#btnDone" ).click(function() {
            ...
            var check = $('input[name="ok[]"]').map(function () {
            return this.value; }).get();

            var notcheck = $('input[name="notok[]"]').map(function () {
            return this.value; }).get();

            var replace = $('input[name="replace[]"]').map(function () {
            return this.value; }).get();
    ...
    xmlhttp.onreadystatechange = function() {
            ...
            xmlhttp.open("GET", "../php/pmchecksheetupdate.php?line="+document.getElementById("stextline").value+"&joborder="+document.getElementById("stextjoborderid").value+"&jobtype=" +document.getElementById("stextjobtype").value+"&machineid="+document.getElementById("machineid").value+"&txtsequence="+JSON.stringify(txtsequence)+"&txtjoblist="+JSON.stringify(txtjoblist)+"&check="+JSON.stringify(check)+"&notcheck="+JSON.stringify(notcheck)+"&replace="+JSON.stringify(replace)+"&Remarks="+JSON.stringify(Remarks), true);
            xmlhttp.send();
