...
class BatchRepair{
	...
	function __construct($account = '',$serialno = ''){
		...
		if(strlen($serialno>0)){
			try{
				$conn->open();
				$dataset = $conn->query("SELECT * FROM dbo.log_repair_batch where account = '".$account."' and cardno = '".$serialno."'");
				...
				if($conn->has_rows($dataset)){
					...
					$this->setCurrQuantity($reader['Quantity']);
	...
	public function addRepair(){
		...
		try{
			...
			$conn->query("INSERT INTO dbo.log_repair_batch VALUES('".$this->getAccount()."', '".$this->getCardno()."', '".$this->getStage()."', '".$this->getMachine()."', '".$this->getCategory()."', '".$this->getDefect()."', '".$this->getLocation()."', '".$this->getDetails()."', '".$this->getStatus()."','".$this->getCurrQuantity()."', '".$this->getAddInfo()."', '".$this->getSerialAffected()."', '".$this->getComponet()."','".$this->getRemarks()."', GETDATE(), '".$this->getRejectUser()."', '".$this->getRepairDate()."', '".$this->getRepairUser()."', GETDATE(), '".$this->getLastUpdatedBy()."')");
	...
	public static function GetAllReject($account,$serialno)
	{
		...
		try{
			$conn->open();
			$dataset =  $conn->query("SELECT * FROM dbo.log_repair_batch where account = '".$account."' and cardno = '".$serialno."_1' and status = 0 order by trackingno" );
			$counter = 0;
			while($reader = $conn->fetch_array($dataset)){

				 $Select = new BatchRepair();
					...
					$Select->setCurrQuantity($reader['Quantity']);
...
public function updateRepair($account,$trackingno,$cardno,$name,$remarks){
		...
		try{
			...
			$conn->query("UPDATE dbo.log_repair_batch SET status = 1, repairDate = GETDATE(), repairUser ='".$name."', lastupdate = GETDATE(), lastupdatedby ='".$name."', remarks='".$remarks."' WHERE account ='".$account."' and trackingno ='".$trackingno."' and cardno ='".$cardno."_1'");
...
public static function countReject($account,$serialno){
		...
		try{
			$conn->open();
			$dataset = $conn->query("SELECT * FROM dbo.log_repair_batch where account ='".$account."' and cardno = '".$serialno."_1' and status = 0");
...
public function updateRejectDetail($account,$cardno,$trackingnos,$cmbrejectcat2,$cmbrejectcode2,$txtlocation2,$txtdetails2){
		...
		try{
			...
			$conn->query("UPDATE dbo.log_repair_batch SET category = '".$cmbrejectcat2."', defect = '".$cmbrejectcode2."', location ='".$txtlocation2."', details = '".$txtdetails2."' WHERE account ='".$account."' and trackingno ='".$trackingnos."' and cardno ='".$cardno."_1'");
	...
	public static function GetAllReject2($account,$serialno)
	{
		...
		try{
			$conn->open();
			$dataset =  $conn->query("SELECT * FROM dbo.log_repair_batch where account = '".$account."' and cardno = '".$serialno."' order by trackingno" );
			$counter = 0;
			while($reader = $conn->fetch_array($dataset)){
		...
		return $result;
			...
			try{
				$conn->open();
				$dataset = $conn->query("SELECT count (status) as status FROM dbo.log_repair_batch where account = '".$account."' and cardno = '".$serialno."_1' and status = 1");
				$counter = 0;
			...
			try{
				$conn->open();
				$dataset = $conn->query("SELECT count (status) as status FROM dbo.log_repair_batch where account = '".$account."' and cardno = '".$serialno."_1' and status = 0");
				$counter = 0;
