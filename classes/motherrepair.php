...
class MotherRepair{
	...
	function __construct($account = '',$serialno = ''){
		...
		if(strlen($serialno>0)){
			try{
				$conn->open();
				$dataset = $conn->query("SELECT * FROM dbo.log_repair_mother where account = '".$account."' and cardno = '".$serialno."'");
	...
	public function addRepair(){
		...
		try{
			...
			$conn->query("INSERT INTO [MES].[dbo].[log_repair_mother] VALUES('".$this->getAccount()."', '".$this->getCardno()."', '".$this->getStage()."',  '".$this->getChildPartno()."','".$this->getMachine()."', '".$this->getCategory()."', '".$this->getDefect()."', '".$this->getLocation()."', '".$this->getDetails()."', '".$this->getStatus()."','".$this->getQuantity()."', '".$this->getAddInfo()."', '".$this->getSerialAffected()."', '".$this->getComponent()."','".$this->getRemarks()."', GETDATE(), '".$this->getRejectUser()."', '".$this->getRepairDate()."', '".$this->getRepairUser()."', GETDATE(), '".$this->getLastUpdatedBy()."')");
	...
	public static function GetAllReject($account,$serialno)
	{
		...
		try{
			$conn->open();
			$dataset =  $conn->query("SELECT * FROM dbo.log_repair_mother where account = '".$account."' and cardno = '".$serialno."_1' and status = 0 order by trackingno" );
			$counter = 0;
			while($reader = $conn->fetch_array($dataset)){

				 $Select = new MotherRepair();
				 	$Select->setTrackingNo($reader['trackingno']);
					$Select->setCardno($reader['cardno']);
					$Select->setStage($reader['stage']);
					$Select->setChildPartno($reader['childpartno']);
...
public function updateRepair($account,$trackingno,$cardno,$name,$remarks){
		...
		try{
			...
			$conn->query("UPDATE dbo.log_repair_mother SET status = 1, repairDate = GETDATE(), repairUser ='".$name."', lastupdate = GETDATE(), lastupdatedby ='".$name."', remarks='".$remarks."' WHERE account ='".$account."' and trackingno ='".$trackingno."' and cardno ='".$cardno."_1'");
...
public static function countReject($account,$serialno){
		...
		try{
			$conn->open();
			$dataset = $conn->query("SELECT * FROM dbo.log_repair_mother where account ='".$account."' and cardno = '".$serialno."_1' and status = 0");
...
public function updateRejectDetail($account,$cardno,$trackingnos,$cmbrejectcat2,$cmbrejectcode2,$txtlocation2,$txtdetails2){
		...
		try{
			...
			$conn->query("UPDATE dbo.log_repair_mother SET category = '".$cmbrejectcat2."', defect = '".$cmbrejectcode2."', location ='".$txtlocation2."', details = '".$txtdetails2."' WHERE account ='".$account."' and trackingno ='".$trackingnos."' and cardno ='".$cardno."_1'");
	...
	public static function GetAllReject2($account,$serialno)
	{
		...
		try{
			$conn->open();
			$dataset =  $conn->query("SELECT * FROM dbo.log_repair_mother where account = '".$account."' and cardno = '".$serialno."' order by trackingno" );
			$counter = 0;
			while($reader = $conn->fetch_array($dataset)){
		...
		return $result;
			...
			try{
				$conn->open();
				$dataset = $conn->query("SELECT count (status) as status FROM dbo.log_repair_mother where account = '".$account."' and cardno = '".$serialno."_1' and status = 1");
				$counter = 0;
