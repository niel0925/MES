...
class Partsrecords{
	...
	public function issuePart()
	{
		...
		public function updateThaw($Startdate,$Enddate,$did,$account)
	{
		...
		try{
			$conn->open();
			$conn->query("UPDATE dbo.kanban_partsrecords SET Thawdate  = '".$Startdate."',Expdate='".$Enddate."' WHERE account ='".$account."' and DID  = '".$did."'");
		...
		public static function StartTime()
	{
		...
		try{
			$conn->open();
			$myquery = $conn->query("SELECT GETDATE() as StartTime");

			if ($conn->has_rows($myquery)>0){
			    $reader = $conn->fetch_array($myquery);
			    $time = $reader['StartTime']->format('Y-m-d h:i:s a');
...
public function didInfo($did)
	{
		...
		try{
			...
			if ($conn->has_rows($myquery)>0){
				...
				$this->setThawdate($reader['Thawdate']->format('Y-m-d'));
				$this->setExpdate($reader['Expdate']->format('Y-m-d h:i:s a'));
			}
	...
	public static function modelSequence($account) {
		...
		try {
			...
			$query = $conn->query("SELECT DISTINCT KSQMODEL FROM dbo.KSEQUENCE_HEADER WHERE KSACCOUNT = '". $account ."' and KSQSTATUS ='1'");
			$count = 0;
	...
	public function thawbakelogs($start,$enddate,$did,$machineId,$compotype,$lastupdateby,$remarks,$account){
		$success = true;
		$conn = new Connection();

		try{
			$conn->open();
			$conn->query("INSERT INTO dbo.kanban_thawbakelogs VALUES( 
								'".$start."',
								'".$enddate."',
								'".$did."',
								'".$machineId."',
